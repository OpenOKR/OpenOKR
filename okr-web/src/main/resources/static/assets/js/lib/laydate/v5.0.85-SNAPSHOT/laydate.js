...
;!function () {
    ...
    var isLayui = window.layui && layui.define, ready = {
        ...
        , laydate = {
            ...
            , ready: function (fn) {
                ...
                // bugfix : 我们的系统是基于requirejs的，计算css路径的时候会错误，而且我们的css是通过依赖配置的方式加载，可以不用这个
                // 所以直接执行回调
        ...
        , ELEM_STATIC = 'layui-laydate-static', ELEM_LIST = 'layui-laydate-list', ELEM_SELECTED = 'laydate-selected', ELEM_HINT = 'layui-laydate-hint', ELEM_PREV = 'laydate-day-prev', ELEM_NEXT = 'laydate-day-next', ELEM_FOOTER = 'layui-laydate-footer', ELEM_SHORTCUT = 'layui-laydate-shortcut', ELEM_CONFIRM = '.laydate-btns-confirm', ELEM_TIME_TEXT = 'laydate-time-text', ELEM_TIME_BTN = '.laydate-btns-time'
    ...
    */
    ...
    Class.prototype.config = {
        ...
        , disabledDate: null // 禁用日期 ["2017-12-15", "12-16（月日）", "13（日）", '2011（年）', "2017-5（年月）", "日一二三四五六（星期）"],
    };
    ...
    Class.prototype.lang = function () {
        var that = this
            ...
            , en: {
                ...
                , format: "Format error"
                , range: "Date out of valid range"
                , reselection: "Please re-select"
            }
        };
    ...
    Class.prototype.render = function () {
        ...
        lay(divFooter).html(function () {
            ...
            if (options.range && that.config.type != 'time') {
    ...
    Class.prototype.checkDate = function (fn) {
        ...
        if (typeof value === 'string' && value) {
            if (that.EXP_IF.test(value)) { //校验日期格式
                ...
                that.hint(that.lang().format + '：<br>' + (
                        options.range ? (options.format + ' ' + options.range + ' ' + options.format) : options.format
                    ) + '<br>' + that.lang().reselection);
                error = true;
            }
    ...
    Class.prototype.limit = function (elem, date, index, time, type) {
        ...
        // 判断特殊限制
        if (options.disabledDate && options.disabledDate instanceof Array && options.disabledDate.length > 0) {
            for (var i = 0, value; i < options.disabledDate.length; i++) {
                value = options.disabledDate[i];
                // 解析值的类型
                if (/^(\d{4})[\s\-\\]+([0-9]{1,2})[\s\-\\]+([0-9]{1,2})$/ig.test(value) && (isDate || isAll)) {
                    // 年月日
                    if (thisDateTime.year == RegExp.$1 && thisDateTime.month == RegExp.$2 && thisDateTime.date == RegExp.$3) {
                        isDisabled = true;
                    }
                } else if (/^(\d{4})[\s\-\\]+([0-9]{1,2})$/ig.test(value) && (isMonth || isAll)) {
                    // 年月
                    if (thisDateTime.year == RegExp.$1 && thisDateTime.month == RegExp.$2) {
                        isDisabled = true;
                    }
                } else if (/^([0-9]{1,2})[\s\-\\]+([0-9]{1,2})$/ig.test(value) && (isDate || isAll)) {
                    // 月日
                    if (thisDateTime.month == RegExp.$1 && thisDateTime.date == RegExp.$2) {
                        isDisabled = true;
                    }
                } else if (/^(\d{4})$/ig.test(value) && (isYear || isAll)) {
                    // 年
                    if (thisDateTime.year == RegExp.$1) {
                        isDisabled = true;
                    }
                } else if (/^([0-9]{1,2})$/ig.test(value) && (isDate || isAll)) {
                    // 日
                    if (thisDateTime.date == RegExp.$1) {
                        isDisabled = true;
                    }
                } else if (/^([日一二三四五六])$/ig.test(value) && (isDate || isAll)) {
                    // 星期
                    var week = that.newDate(thisDateTime).getDay();
                    if ('日一二三四五六'.indexOf(value) == week) {
                        isDisabled = true;
                    }
                }
            }
        ...
        elem && elem[(isOut || isDisabled) ? 'addClass' : 'removeClass'](DISABLED);
        return (isOut || isDisabled);
    };
                ...
                lay(ul).find('ol').each(function (i, ol) {
                    lay(ol).find('li').each(function (ii, li) {
                        that.limit(lay(li), [{
                            ...
                            , seconds: ii
                        }][i], index, [['hours'], ['hours', 'minutes'], ['hours', 'minutes', 'seconds']][i], type);
                    });
                });
                if (!options.range) that.limit(lay(that.footer).find(ELEM_CONFIRM), that[startEnd], 0, ['hours', 'minutes', 'seconds'], type);
            };
            if (options.range) {
        ...
        if (type === 'year' || type === 'month') {
            ...
            lay(ul).find('li').on('click', function () {
                ...
                if (index === 0) {
                    ...
                    that.limit(lay(that.footer).find(ELEM_CONFIRM), null, 0, null, type);
    ...
    Class.prototype.tool = function (btn, type) {
        var that = this
            ...
            , now: function () {
                ...
                var curTime = that.newDate(dateTime).getTime();
                var maxTime = that.newDate(options.max).getTime();
                var minTime = that.newDate(options.min).getTime();

                if (curTime > maxTime || curTime < minTime) {
                    that.setValue("");
                    that.hint(that.lang().range + '：<br>' + that.parse(1, options.min) + ' ~ ' + that.parse(1, options.max) + '<br>' + that.lang().reselection)
                } else {
                    that.setValue(that.parse()).remove();
            ...
            , confirm: function () {
                if (options.range) {
                    ...
                    if (lay(btn).hasClass(DISABLED)) return that.hint(that.lang().range);
                }
                that.done();
                that.setValue(that.parse()).remove()
            }
    ...
    Class.prototype.events = function () {
        ...
        if (!options.range) {
            lay(options.elem[0]).on('keyup', function (e) {
                ...
                var formatKey = (that.config.format || 'yyyy-MM-dd').match(/(y+|M+|d+)/g); // 解析format，得到对应匹配符的顺序数组
                var _partValue = '';

                for (var i = 0, preMatchLength = 0, matchLength = 0; i < formatKey.length; i++) {
                    preMatchLength += i && formatKey[i - 1].length; // 记录要截取用来匹配的字符串
                    matchLength += formatKey[i].length;
                    if (value.length == matchLength) {
                        _partValue = value.substring(preMatchLength, matchLength); // 获取当前值
                        // 设置日历
                        if (formatKey[i] == 'yyyy') {
                            dateTime.year = parseInt(_partValue);
                        } else if (formatKey[i] == 'MM') {
                            dateTime.month = parseInt(_partValue) - 1;
                        } else if (formatKey[i] == 'dd') {
                            dateTime.date = parseInt(_partValue);
                        }
                    }
                }
                that.change(0).refresh();
                if (value.length >= that.config.format.replace(/[\s\-\\\/]/g, '').length) {
                    that.setValue(that.parse(1, dateTime));

                    var _curSelectTIme = that.newDate(dateTime).getTime();
                    var maxTime = that.newDate(options.max).getTime();
                    var minTime = that.newDate(options.min).getTime();

                    if (_curSelectTIme > maxTime || _curSelectTIme < minTime) {
                        that.setValue("");
                        that.hint(that.lang().range + '：<br>' + that.parse(1, options.min) + ' ~ ' + that.parse(1, options.max) + '<br>' + that.lang().reselection)
                    }
                }
    ...
    Class.prototype.selectDate = function (type, count) {
        var that = this;
        var currentDate = new Date();
        ...
        that.endDate = {
            year: that.startDate.year - (type == "year" ? count : 0)
            , month: that.startDate.month - (type == "month" ? count : 0)
            , date: that.startDate.date - (type == "day" ? count : 0)
            , hours: that.startDate.hours - (type == "hours" ? count : 0)
            , minutes: that.startDate.minutes - (type == "minutes" ? count : 0)
            , seconds: that.startDate.seconds - (type == "seconds" ? count : 0)
        };
    ...
    isLayui ? (
        ...
        }(); // bugfix去除amd的支持，还是因为我们系统脚本的加载方式
    /*(
