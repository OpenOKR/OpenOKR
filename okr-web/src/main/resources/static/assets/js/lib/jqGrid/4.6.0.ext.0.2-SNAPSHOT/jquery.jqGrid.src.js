...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
        ...
        //TODO:begin重写 让配置的rowList覆盖默认值
        if(pin.rowList) {
            p.rowList = pin.rowList;
        }
        //TODO:end重写 让配置的rowList覆盖默认值
		var ts= this, grid={
			headers:[],
			cols:[],
			footers: [],
...
$.jgrid.extend({
	...
	showHideCol : function(colname,show) {
		return this.each(function() {
			...
			if( gh )  {
				if ($t.p.groupHeaders && $t.p.groupHeaders.length > 0) {
                    $t.p.groupHeaders.forEach(function(item,idx) {
                        $($t).jqGrid('setGroupHeaders', item);
                    });
				} else {
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			case "custom" :
				...
				$(elem).addClass("sj-custom");//added by qfZhu
				try {
