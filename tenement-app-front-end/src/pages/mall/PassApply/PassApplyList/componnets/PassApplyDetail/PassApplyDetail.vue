        ...
        <div class="applydetail-area">
          <el-row>
            ...
            <el-col :span="8">
              <div class="grid-content-top">
                <p class="key">零售类放行</p>
                <p class="value">{{totalData.retailApplyNum||0}}</p>
              </div>
            </el-col>
            <el-col :span="8">
              <div class="grid-content-top">
                <p class="key">装修类放行</p>
                <p class="value">{{totalData.decorateApplyNum||0}}</p>
              </div>
            </el-col>
          </el-row>
          <!---->
          <table-comb
                  ...
                  :get-action="$api.mall.passApply.getPassApplyDetail"
  ...
  export default {
    ...
    methods: {
      ...
      exportExcel(){
        ...
        vo.token = this.token;
        this.$api.mall.passApply.exportApplyDetailStatistic(vo)
      },


      //获取业态列表
      getShopTypeOptions(){
        this.$api.mall.passApply.shopTypeOptions(this.siteId).then(res => {
          this.shopTypeOptions = res.data;
        });
      },
      afterFetchData(){
        let searchVo = this.$refs.tableMain.getPageVo();
        this.$api.mall.passApply.getDetailTopStatisticData(searchVo).then(res => {
