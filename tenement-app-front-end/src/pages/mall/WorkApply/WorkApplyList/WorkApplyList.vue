<template>
  <div class="workapply-area">
    <d2-container>
      <template slot="header">加班申请</template>
      <el-row>
        <el-col :span="8">
      ...
      <table-comb
              ...
              :get-action="$api.mall.workApply.getWorkApplyList"
              :get-action-where="getActionWhere"
              :afterFetchData="afterFetchData"
      >
        <!--基础查询-->
        <template slot="baseSearchForm" slot-scope="scope">
          <div class="fl fwb">加班统计汇总</div>
          申请时间:
          <el-date-picker
        ...
        <template slot="tableColumns">
          ...
          <el-table-column label="操作" width="200px">
            <template slot-scope="props">
              <el-button type="text" size="mini" @click="openWorkListDialog(props.row)">查看详情</el-button>
            </template>
          </el-table-column>
        </template>
      </table-comb>
    ...
    <work-apply-detail ref="workListDialog"></work-apply-detail>
...
<script>
  ...
  import WorkApplyDetail from "./componnets/WorkApplyDetail";
  export default {
    // 如果需要缓存页面
    // name 字段需要设置为和本页路由 name 字段一致
    name: "WorkApplyList",
    components: {WorkApplyDetail},
    mixins: [
      listMixin
    ],
    data() {
    ...
    methods: {
      ...
      exportExcel(){
        ...
        vo.token = this.token;
        this.$api.mall.workApply.exportApplyStatistic(vo)
      },

      // 查看详情页面
      openWorkListDialog(row) {
      ...
      afterFetchData(){
        ...
        this.$api.mall.workApply.getTopStatisticData(searchVo).then(res => {
...
<style>
  .workapply-area .table-filter{text-align: right}
  .workapply-area .table-filter .fl{line-height: 40px}
  .workapply-area .table-filter .fwb{font-weight: bold}
  .workapply-area .d2-container-full__body{padding:0}
  .workapply-area .el-row{  background-color: #f2f9ff;
    border-radius: 3px;
    padding: 20px 0 0 0;}
  .workapply-area .m-table-comb{padding:20px 0 0 0}
</style>
