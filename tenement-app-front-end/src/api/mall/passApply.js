...
export default {
   ...
   * @returns {*}
   */
  getPassApplyList (condition, pageInfo) {
    ...
    return axios.post('/api/mall/passApply/getApplyStatisticPage.json', {
      ...condition,
      ...pageInfo
  })
  },
   ...
   * @returns {*}
   */
  getPassApplyDetail (condition, pageInfo) {
    pageInfo = Object.assign({
      currentPage: 1,
      pageSize: 10
    }, pageInfo)
    ...
    return axios.post('/api/mall/passApply/getApplyDetailStatisticPage.json', {
      ...condition,
      ...pageInfo
    })
  },
  ...
  exportApplyStatistic (vo) {
    ...
    window.open(`${process.env.VUE_APP_API_PREFIX}/api/mall/passApply/exportApplyStatistic.htm?${pargam}`,'_blank');
  },
  /**
   * Excel导出 放行申请表详情
   * @returns {*}
  ...
  exportApplyDetailStatistic (vo) {
    ...
    let pargam = setUrlK(vo);
    window.open(`${process.env.VUE_APP_API_PREFIX}/api/mall/passApply/exportApplyDetailStatistic.htm?${pargam}`,'_blank');
  },
  /**
   * 获取统计页面顶部统计数据
   * @returns {*}
  ...
  getTopStatisticData (vo) {
    ...
    return axios.post('/api/mall/passApply/getTopStatisticData.json', vo)
  },
  /**
   * 获取详情页面顶部统计数据
   * @returns {*}
  ...
  getDetailTopStatisticData (vo) {
    return axios.post('/api/mall/passApply/getDetailTopStatisticData.json', vo)
  },
